...
export default function HomePage() {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          title="Remittance Volume by Corridor (Monthly)"
        />
        <Chart
          ...
          type="bar"
          xKey="category"
          yKeys={[{ key: 'count', name: 'Transactions (K)' }]}
          title="Transaction Count by Source Country"
        />
      </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          type="pie"
          xKey="label"
          yKeys={[{ key: 'value', name: 'vs Mid-Market %' }]}
          title="FX Rate Competitiveness"
        />
        <ActionMemo
          persona={{ name: 'Ricardo Dela Rosa', role: 'VP Cross-Border Payments' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: 'Launch instant remittance for Saudi→PH corridor', 'Negotiate better FX rates with correspondent in UAE', 'Comply with BSP Circular 1160 on pricing transparency',
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which corridors have the longest settlement times?',
          'Show market share vs competitors by corridor',
          'What is the FX margin trend for USD/PHP?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Corridor Analytics', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'FX & Pricing', icon: '⚡', content: domainTab2 },
